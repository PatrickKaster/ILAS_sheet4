				...
				\item 
					\begin{description}
						...
						\item[Identity of indiscernibles] Cleary the Hamming distance sums to zero iff $x_r,x_s$ don't differ in more than one entry:\\ 
															$D_{\mbox{Hamming}}=0 \Rightarrow  x_r=x_s$
						\item[Symmetry] 
							\begin{alignat*}{1}
								 D_{\mbox{Hamming}}(x_{r},x_{s}) & =\sum_{j=1}^{m}\left|x_{rj}-x_{sj}\right| \\
								 								 & =\sum_{j=1}^{m}\left|x_{sj}-x_{rj}\right|=D_{\mbox{Hamming}}(x_{s},x_{r})
							\end{alignat*}
						\item[triangle inequality]
							\begin{align*}
								\forall x_{r},x_{s},x_{t}:\\
								D_{\mbox{Hamming}}(x_{r},x_{t}) & = \sum_{j=1}^{m}\left|x_{rj}-x_{tj}\right|\\
																...
																& = D_{\mbox{Hamming}}(x_{r},x_{s})+D_{\mbox{Hamming}}(x_{s},x_{t})
							\end{align*}
					\end{description}
				\item 
					\begin{description}
